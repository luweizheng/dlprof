...
parser.add_argument('-a', '--arch',
                    metavar='ARCH',
                    default='resnet50',
                    help='model architecture: resnet50 for 224 or resnet56 for 32')
parser.add_argument('--input_size',
                    type=int,
                    default="224",
                    help='input size of the dataset. we should rescale it to the input size we want.')
# apex
...
def main_worker(args):
    ...
    print(f"train_start_ts: {train_start_ts}")
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"[metrics] epoch: {epoch}, walltime: {time.time()}, fps: {fps:.5f}, val_acc1: {acc1:.5f}, val_loss: {val_loss:.5f}", flush=True)
    ...
    duration = time.time() - train_start_ts
    print(f"total_duration: {duration } s")
    print(f"total_duration: {duration / (60)} min")
